...
let forecastData = async () => {
	...
	*/
	...
	csv = csv.map((row, index) => {
		...
		return {
			...
			//----- TO DO ----Add Row Counter here
			Row: index + 1,
			//----- TO DO ----Add O = Original Data.
			Dtype: "O",
			Method: variableMapping,
			//----- TO DO ---- Add Fields being used in forecast or ZERO (0) fill.
			//"Features" : forecastColumns.join(),
	...
	*/
	...
	fs.writeFileSync(
		...
		`${Object.keys(data[0][0]).slice(0,12).join(",")},"|${forecastColumns.slice(0,forecastColumns.length-1).join(',')}|"\n`
	);
	...
	for (let i = 0; i < systems.length; i++) {
		...
		for (let j = 0; j < data[i].length; j++) {
			...
			row = Object.values(data[i][j]).slice(0,12).join(",");
	...
	*/
	...
	for (let i = 0; i < systems.length; i++) {
		...
		for (let j = 0; j < forecastsForAllSystems[i].length; j++) {
			...
			row += `${forecastData[i]["COUNTER"].length==1 ? forecastData[i]["COUNTER"][0] : forecastData[i]["COUNTER"][j]},`
			row += `${forecastColumns.includes("FltMet1") ? forecastsForAllSystems[i][j][0].toFixed(2) : 0},`;
			row += `${forecastColumns.includes("FltMet2") ? forecastsForAllSystems[i][j][0].toFixed(2) : 0},`;

			//----- TO DO ----  This MUST ALWAYS include FLOAT 1 and FLOAT 2 columns; even if not a feature. It should be included as zero.
			//Forecast Output

			//----- TO DO ---- This MUST ALWAYS include RAT 1 and RAT 2 columns; even if not a feature. It should be included as zero.
			row += `${
				forecastColumns.includes("FltMet1")
					? calculateRatio(
							containsCOUNTERInMapping
								? forecastFeatures[i][j][0]
								:forecastData[i]["COUNTER"].length==1 ? forecastData[i]["COUNTER"][0] : forecastData[i]["COUNTER"][j],
							containsUNITSInMapping
								? forecastFeatures[i][j][2]
								: forecastData[i]["UNITS"].length==1 ? forecastData[i]["UNITS"][0] : forecastData[i]["UNITS"][j],
							Period,
